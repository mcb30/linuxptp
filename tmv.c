...
void tmv_to_Timestamp(tmv_t x, struct Timestamp *ts)
{
	uint64_t seconds = x.ns / NS_PER_SEC;
	ts->seconds_lsb = seconds;
	ts->seconds_msb = (seconds >> 32);
	ts->nanoseconds = (x.ns % NS_PER_SEC);
}
...
tmv_t hwts_to_tmv(const struct hw_timestamp *hwts)
{
...
tmv_t hwts_sw_to_tmv(const struct hw_timestamp *hwts)
{
